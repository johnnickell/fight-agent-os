...
2. **Challenge acceptance.** Inspect the actual diff, map each criterion to changed behavior and tests, pursue unsupported claims, and exercise applicable areas in the review standards. Run fresh focused checks and the full gate when needed; distinguish new results from inherited receipts and disclose warnings, skips and limits. A rebase is judged on its effective new-base diff, not rejected solely for changed commit IDs.
3. **Classify.** Give each actionable finding severity, affected criterion, reproducible evidence, expected/observed behavior, and correction. Separate non-blocking findings and residual uncertainty. `accept` only when every criterion has sufficient evidence and there are no blockers; otherwise `revise`.
