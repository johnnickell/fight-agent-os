...
[Define the coordinated TASK authority protocol (WF-015)](tickets/WF-015-define-coordinated-task-authority-protocol.md) is the one recommended next decision. Repository context, browser ownership, and the Harness resource, profile, credential, and trusted-origin boundaries are settled; WF-015 can now assign concrete coordinator, planner, worker, reviewer, and publisher responsibilities, choose the first managed Agent profile templates, and define bounded revision loops without inventing transport or runner behavior.
...
- Exact managed Agent profile catalog, coordinator process topology, model/provider assignment, cost budgets, retry values, scheduling fairness, and concurrency limits.
