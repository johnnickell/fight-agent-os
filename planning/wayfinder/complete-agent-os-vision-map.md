...
[Define execution history and event authority (WF-017)](tickets/WF-017-define-execution-history-and-event-authority.md) is the one recommended next decision. The single-TASK workflow and its container-native runner, durable PostgreSQL dispatch, concurrent isolated worktrees, leases, recovery, sandbox, brokered credentials, and external-tool boundary are settled; WF-017 can now assign authoritative aggregates, observational events, Pi conversation references, artifacts, retention, and Dashboard projections without redefining process ownership.
...
- Real-time browser transport, artifact storage backend, log indexing/search, and long-term execution-history retention; WF-017 owns these choices.
- Complete local Compose, shared ingress, hostname, and optional infrastructure topology; WF-023 owns these choices, while isolated-worktree preview URLs remain a future consideration rather than a promised capability.
