...
[Define skill trust and harness distribution (WF-014)](tickets/WF-014-define-skill-trust-and-harness-distribution.md) is the one recommended next decision. The stable MCP Skills obligations, current SDK/client/server support, and viable Pi seams are now evidenced; WF-014 can select identity, approval, cache, collision, packaging, and bootstrap behavior without assuming unsupported native Pi or fully mature SDK support.
