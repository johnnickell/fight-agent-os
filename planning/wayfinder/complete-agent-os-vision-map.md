...
[Prototype the sessions-first browser experience (WF-018)](tickets/WF-018-prototype-sessions-first-browser-experience.md) is the one recommended next decision. Workflow, Review, conversation, attempt, observation, usage, Artifact, retention, and projection authority are settled; WF-018 can now test a compact repository-scale session overview and lazy operational dossier—including the immutable syntax-highlighted Git diff—without inventing data or embedding transcript history.
...
- Exact Mercure topic and reconnect behavior, Dashboard pagination and detail composition, log indexing/search, and safe artifact-preview interactions; WF-018 owns the disposable browser evidence rather than production implementation.
