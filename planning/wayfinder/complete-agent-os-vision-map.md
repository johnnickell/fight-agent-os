...
[Define browser instruction inspection and assisted editing (WF-022)](tickets/WF-022-define-browser-instruction-inspection-and-assisted-editing.md) is the one recommended next decision. Repository context, conversation ownership, runner custody, onboarding, and creation/registration are settled, so browser instruction work can now define safe reading, revision-checked human and AI proposals, explicit filesystem and Git effects, and recovery without becoming a general-purpose IDE.
...
- Whether newly exposed complexity warrants the remaining candidate child maps above and which new decisions each would own; WF-020 rejected a platform-onboarding child map and WF-021 rejected a starter/provisioning child map unless later discovery exposes incompatible behavior.
...
- Exact model assignments, OpenRouter allowlists and cost-budget values for browser Agents, coding-Agent provider choices, and per-repository resource-lock catalogs beyond the accepted server-side OpenRouter support, three-consumer default, and bounded attempt policy.
