...
[Research MCP Skills and Pi support (WF-013)](tickets/WF-013-research-mcp-skills-and-pi-support.md) is the one recommended next decision. Repository context, the authoritative Planning model, and the Markdown cutover protocol are now settled, while factual MCP/Pi capability research can proceed independently and unlock the skill/harness decision before coordinated work depends on an unsupported transport. WF-012 is also dependency-eligible but remains a separate browser-product decision.
