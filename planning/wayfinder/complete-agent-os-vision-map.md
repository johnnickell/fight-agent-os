...
[Define new project creation and registration (WF-021)](tickets/WF-021-define-new-project-creation-and-registration.md) is the one recommended next decision. Repository identity, policy precedence, runner custody, local runtime, shared ingress, host-mutation boundaries, and the developer onboarding path are settled, so project creation can now define its filesystem, starter, source-host, checkout, context, and registration effects without hiding partial outcomes.
...
- Whether newly exposed complexity warrants the remaining candidate child maps above and which new decisions each would own; WF-020 rejected a platform-onboarding child map unless real qualification exposes incompatible behavior, while WF-021 will decide whether starter qualification and project provisioning require one.
