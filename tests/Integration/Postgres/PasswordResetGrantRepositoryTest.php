...
/**
 * Class PasswordResetGrantRepositoryTest
 */
...
TRUNCATE password_reset_grants, refresh_session_used_credentials, refresh_sessions,
    user_role_assignments, user_email_claims, users, role_permissions, roles, permissions CASCADE
SQL
        );
    ...
    public function testRoundTripDeliveryLifecycleDueOrderAndSecretAbsence(): void
    {
        ...
        self::assertSame(
            ...
            $this->grants->getByDeliveryId($grant->getDelivery()->getId())?->getId()->toString()
        );
        ...
        self::assertSame(
            ...
            $this->grants->findDue($this->now, 10)[0]->getDeliveryId()->toString()
        );
        ...
        $failed = $rehydrated->failDelivery(
            $claim,
        ...
        self::assertTrue($this->commit(
            fn(): bool => $this->grants->replace($retry, $retry->requestDeliveryRetry())
        ));
        ...
        $nextClaim = $current->claimDelivery(
            $nextToken,
            $current->getDelivery()->getDueAt(),
    ...
    /**
     * Verifies due discovery is bounded ordered and excludes old generations
     */
    public function testDueDiscoveryIsBoundedOrderedAndExcludesOldGenerations(): void
    {
...
SQL, [$otherUser->toString(), $this->userId->toString()]);
        ...
        self::assertSame($expected, array_map(
            static fn($item): string => $item->getDeliveryId()->toString(),
    ...
    /**
     * Verifies terminal append and successor fence historical digests
     */
    public function testTerminalAppendAndSuccessorFenceHistoricalDigests(): void
    {
        ...
        self::assertFalse($this->commit(
            fn(): bool => $this->grants->appendAfterTerminal($terminal, $this->grant()[0])
        ));
        self::assertSame(
        ...
        self::assertFalse($this->commit(
            fn(): bool => $this->grants->replaceWithSuccessor($candidate, $terminalCandidate, $reused)
        ));
    ...
    /**
     * Verifies succession rejects stale completion and rolls back after second write failure
     */
    public function testSuccessionRejectsStaleCompletionAndRollsBackAfterSecondWriteFailure(): void
    {
        [$first] = $this->grant();
        self::assertTrue($this->commit(fn(): bool => $this->grants->add($first)));
        [$successor] = $this->grant();
        ...
        self::assertFalse($this->commit(
            fn(): bool => $this->grants->replaceWithSuccessor($first, $terminal, $this->grant()[0])
        ));
        self::assertEquals($terminal->getRevokedAt(), $this->grants->getById($first->getId())?->getRevokedAt());
    }

    /**
     * Verifies missing user and cross purpose are isolated
     */
    public function testMissingUserAndCrossPurposeAreIsolated(): void
    {
        ...
        self::assertFalse($this->connection->fetchOne(
            'SELECT id FROM password_reset_grants WHERE id = ?',
            [PasswordResetGrantId::generate()->toString()]
        ));
        self::assertSame(0, (int) $this->connection->fetchOne(
            'SELECT COUNT(*) FROM password_reset_grants WHERE user_id != ?',
            [$this->userId->toString()]
        ));
    }

    /**
     * Verifies second write constraint failure rolls back terminalization
     */
    public function testSecondWriteConstraintFailureRollsBackTerminalization(): void
    {
        [$first] = $this->grant();
        self::assertTrue($this->commit(fn(): bool => $this->grants->add($first)));
        $terminal = $first->revoke($this->now->modify('+1 minute'));
        ...
        [$fresh] = $this->grant();
        $duplicateDelivery = PasswordResetDelivery::create(
            $first->getDelivery()->getId(),
            $this->userId,
        ...
        $duplicate = PersistedPasswordResetGrant::reconstitute(
            $fresh->getId(),
            $this->userId,
        ...
        self::assertFalse($this->commit(
            fn(): bool => $this->grants->replaceWithSuccessor($middle, $nextTerminal, $duplicate)
        ));
        self::assertSame(0, $this->grants->getById($middle->getId())?->getRevision());
        self::assertNull($this->grants->getById($duplicate->getId()));
        self::assertSame(2, (int) $this->connection->fetchOne('SELECT COUNT(*) FROM password_reset_grants'));
        self::assertTrue($this->commit(
            fn(): bool => $this->grants->replaceWithSuccessor($middle, $nextTerminal, $fresh)
        ));
    }

    /**
     * Verifies fabricated predecessor and duplicate completion do not write
     */
    public function testFabricatedPredecessorAndDuplicateCompletionDoNotWrite(): void
    {
        [$first] = $this->grant();
        self::assertTrue($this->commit(fn(): bool => $this->grants->add($first)));
        $fake = PersistedPasswordResetGrant::reconstitute(
            $first->getId(),
            $this->userId,
            ...
            PasswordResetDelivery::create(
                $first->getDelivery()->getId(),
                $this->userId,
                $first->getDelivery()->getEmail(),
                'different-encrypted-material',
                $first->getExpiresAt(),
                $this->now
            ),
            null,
    ...
    /**
     * Verifies competing completions allow one terminal revision
     */
    public function testCompetingCompletionsAllowOneTerminalRevision(): void
    {
        [$first] = $this->grant();
        self::assertTrue($this->commit(fn(): bool => $this->grants->add($first)));
        $consumed = $first->consume($this->now->modify('+1 minute'));
    ...
    /**
     * Verifies competing terminal append waits and rejects stale predecessor
     */
    public function testCompetingTerminalAppendWaitsAndRejectsStalePredecessor(): void
    {
        [$first] = $this->grant();
        self::assertTrue($this->commit(fn(): bool => $this->grants->add($first)));
        $terminal = $first->consume($this->now->modify('+1 minute'));
    ...
    /**
     * Verifies competing terminal operations wait and only one successor wins
     */
    public function testCompetingTerminalOperationsWaitAndOnlyOneSuccessorWins(): void
    {
        [$first] = $this->grant();
        self::assertTrue($this->commit(fn(): bool => $this->grants->add($first)));
        [$winner] = $this->grant();
    ...
    private function grant(?PasswordResetCredential $credential = null, ?UserId $userId = null): array
    {
        ...
        return [PasswordResetGrant::issue(
            ...
            EmailAddress::fromString('reset@example.test'),
            'encrypted-material'
        ), $credential];
    }
