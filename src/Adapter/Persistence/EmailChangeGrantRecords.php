...
final class EmailChangeGrantRecords
{
    ...
    public static function hydrate(array $row): EmailChangeGrant
    {
        ...
        return PersistedEmailChangeGrant::reconstitute(
            EmailChangeGrantId::fromString((string) $row['id']), $userId, (string) $row['credential_digest'],
            self::parse($row['expires_at']), $delivery, self::parseOptional($row['consumed_at']),
            self::parseOptional($row['revoked_at']), self::parseOptional($row['expired_at']), (int) $row['revision']
        );
    }
