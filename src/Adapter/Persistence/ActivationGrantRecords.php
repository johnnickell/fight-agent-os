...
final class ActivationGrantRecords
{
    ...
    public static function hydrate(array $row): ActivationGrant
    {
        ...
        $delivery = PersistedActivationDelivery::reconstitute(
            ...
            $row['delivery_ciphertext'] === null
                ? null : EncryptedCredentialMaterial::fromString((string) $row['delivery_ciphertext']),
            $expiresAt,
            new DateTimeImmutable((string) $row['delivery_due_at']),
            CredentialDeliveryStatus::from((string) $row['delivery_status']),
            $row['delivery_claim_token'] === null ? null
                : CredentialDeliveryClaimToken::fromString((string) $row['delivery_claim_token']),
            ...
            $row['delivery_last_failure'] === null ? null
                : CredentialDeliveryFailure::from((string) $row['delivery_last_failure'])
        );
